...
PCIe Bifurcation和PLX芯片辨析
===============================

PCIe Bifurcation实际上是CPU内置的功能，完全依赖CPU控制，数据交换必须经过CPU。本质是CPU的PCIe控制器(Root Complex)如何配置其引脚的逻辑映射，这种拆分是纯逻辑的: CPU告诉它的控制器"从引脚 A 到 B 的这 4 条通道现在作为一个独立的设备端口"。

注意：主板并不提供拆分能力，它只是通过BIOS提供一个开关，并确保主板上的走线物理上支持这种拆分(例如将x16插槽连接到能识别不同时钟信号的底座上)。

PLX芯片则是一个 **独立的硬件交换芯片** : PLX芯片就像一个网络交换机(Switch)，它通过16条通道连接到CPU，但是在后端可以"变"出32条甚至更多通道。不同于Bifurcation静态划分带宽，PLX芯片可以动态分配带宽。如果两个GPU正在相互通讯(P2P)，则数据包会在PLX芯片内部进行交换，而不需要占用CPU的带宽资源。

这也就是为何高端工作站主板或专门的PCIe扩展卡通过PLX交换芯片能够为 GPU 设备提供性能超越常规PCIe交换数据，在这种硬件配置下虽然不如NVIDIA NVLink性能那么强悍，但是也是一种低成本扩展GPU连接并提高交换性能的解决方案。

外置PCIe switch扩展坞
--------------------------

我在淘宝上查询了一下，这种PLX解决方案有商家提供全套 "外置PCIe switch扩展坞" ，可以将普通工作站扩展成安装高达4个GPU设备，但是价格不菲: 下图不含电源的扩展坞 5800 元(PCIe 4)

.. figure:: ../../../../_static/linux/server/hardware/hpe/pcie_switch.png

.. figure:: ../../../../_static/linux/server/hardware/hpe/pcie_switch_all.png

不过这种外置扩展坞采用了机架服务器的暴力风扇，噪音极大，无法在家中使用。此外，我感觉不如直接购买品牌的二手8GPU机架服务器性价比更高。
