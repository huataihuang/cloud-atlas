.. _alpine_install_mba11_late_2010_detail:

====================================================
MacBook Air 11" Late 2010安装Alpine Linux折腾笔记
====================================================

这次 :ref:`alpine_install_mba11_late_2010` 遇到了不少之前没有遇到过的异常问题，一些技术细节是早期硬件带来的困扰，虽然随着硬件进步可能再也不会遇到，但是折腾过程中学到的知识还是值得整理和记录下来...
...
.. literalinclude:: alpine_install_mba11_late_2010_detail/dd
   :caption: 制作启动U盘
...
gemini建议采用 :ref:`ventoy` ，Ventoy 对老旧 Mac 固件的 EFI 兼容性比原始 dd 模式要强得多。

在使用 Ventoy 之前我也尝试了手动解压方法: 直接将 U 盘格式化为 FAT32，然后把 ISO 里的内容解压进去

.. literalinclude:: alpine_install_mba11_late_2010_detail/tar
...
.. literalinclude:: alpine_install_mba11_late_2010_detail/uninstall_refind
   :caption: 删除rEFInd
...
- 我的 :ref:`mba11_late_2010` 太古老了，EFI协议应该是早期版本， :strike:`无法处理现代Linux发行版的 iso-hybrid 格式启动，甚至也无法启动 ventoy 现在提供的启动ISO镜像` gemini提供的参考信息参考如下:

  - :ref:`mba11_late_2010` 处于32位到64位EFI的过渡阶段，其EFI虽然是64位的，但是在处理 NVIDIA 320M的显示存在缺陷: 2010 款 MacBook Air (NVIDIA 320M) 最经典的固件硬件坑之一是 **该款 Mac 的 64 位 EFI 固件在加载某些第三方的 x64 EFI 驱动(如 xfs_x64.efi )或者尝试通过纯 EFI 模式唤醒 NVIDIA 320M 显卡时，会直接卡死在初始化阶段，导致黑屏/灰屏。**
...
.. literalinclude:: alpine_install_mba11_late_2010_detail/dd_3.14
...
.. literalinclude:: alpine_install_mba11_late_2010_detail/dd_3.2
   :caption: 制作3.2系列启动U盘
...
.. literalinclude:: alpine_install_mba11_late_2010_detail/dd_dmg
