...
- 超越 ``FAT32/VFAT`` ，已经 **没有单个文件4G限制** ，这对现代的大文件交换非常有利(不过FAT32兼容性更好，被更多操作系统和设备支持，且适合小型设备，因为FAT32简单而轻量级)
- ``ExFAT`` 性能比 ``FAT32`` 好，特别是大文ä¶传输
- ``ExFAT`` 为移动设备(USB设备，SD卡等)设计，但ä建议用于内部存储(内置硬盘或SSD)
