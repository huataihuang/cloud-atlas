...
我在 :ref:`archlinux_zfs-dkms` 遇到一个麻烦: :ref:`asahi_linux` 11月22日发布新版本内核 ``6.1rc6`` 已经超出了 :ref:`zfs` 支持的最高版本 ``6.0`` ，导致 :ref:`dkms` 编译时候出现异常(但是我感觉很可能是版本编号的支持问题)，所以我准备等 :ref:`asahi_linux` 出正式内核版本时再做尝试。目前先把内核版本回滚到之前的 ``5.19`` 。
