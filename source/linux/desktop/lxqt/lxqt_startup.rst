...
:ref:`light_terminal` 选择使用 :ref:`qterminal` : 需要精心配置，特别是选择 :ref:`qterminal_chinese_font`
...
- 全面支持pdf和epub的轻量级阅读器则可以选择 :ref:`mupdf` (在LXQt我还是分别采用了 ``xpdf`` 和 ``fbreader`` )

.. note::

   我最终还是统一到 :ref:`mupdf` 来阅读电子书，原因:

   - MuPDF 实际上是一个电子书引擎，很多电子书工具都基于它来实现，所以这个软件功能非常强大
   - 全部采用快捷键控制，一旦掌握非常方便使用(类似于vim)
