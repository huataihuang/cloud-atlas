...
- :ref:`fcitx_sway` 输入法可以非常完美使用中文，但是需要注意
  ...
  - :strike:`默认 foot 终端虽然能够输入中文，但是无法现实中文候选字词，所以改为 qterminal` (默认 :ref:`kali_linux` 的 :ref:`xfce` 配套终端模拟器，轻量级且功能完备)可以正常输入中文
  - **2025年** 社区已经将 ``sway-im`` 中文支持补丁合并，现在最佳终端是 :ref:`foot` ，已经完美支持 :ref:`fcitx` 中文输入
  - (我不清楚现在状态，因为我现在只使用 :ref:`firefox` ) ``chromium`` 无法输入中文(显示运行没有问题)，不过好在 ``firefox`` 使用中文输入正常，所以我同时采用这2个浏览器取长补短
