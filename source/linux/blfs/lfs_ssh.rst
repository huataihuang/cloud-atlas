...
我在检查 ``/etc/shadow`` 文件时意外发现 ``admin`` 账号没有设置过密码。我原本以为我强制通过密钥认证可以不用设置这个普通用户密码，但是实践发现， **用户账号必须设置密码，即使SSH是通过密钥认证登陆**
...
这个不允许用户空密码原来不是指ssh客户端连接以后，用户输入的空密码，而是服务器端用户账号本身就不允许空密码。只要服务器上的用户密码没有设置，ssh就禁止该账号登陆，哪怕SSH客户端密钥认证是正确的也不行。 **空密码校验逻辑在前**
