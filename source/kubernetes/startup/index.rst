...
- 如果你已经对Kubernetes有实践经验，例如在公司内部使用过Kubernetes集群并且有一定的服务器部署基础，则建议参考 :ref:`kubernetes_deployment` 部署基于虚拟机环境的模拟真实生产环境的Kubernetes集群，然后再学习本章节快速起步。
...
本章节的后半部分则在较为真实的 :ref:`kubernetes_deployment` 集群上实践部署真正可对外提供服务的系统。
