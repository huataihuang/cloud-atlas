...
在 `glusterfs下载网站 <https://download.gluster.org/pub/gluster/glusterfs/>`_ 可以看到对不同GlusterFS版本的下载索引，例如 :ref:`deploy_centos7_suse15_gluster9` 采用 GlusterFS 9.x ，就可以根据说明看到实际软件包是由 CentOS Storage SIG 提供，也就是首先激活SIG(在 :ref:`build_install_glusterfs` 也同样需要激活):
