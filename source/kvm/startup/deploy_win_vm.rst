...
.. note::
   ...
   在安装过程中使用 ``virtio-win.iso`` 安装磁盘驱动，该磁盘驱动位于 ``amd64`` 目录下，只需要安装一个virtio磁盘驱动就可以完成Windows操作系统安装。等Windows安装完成后，再通过device manager补充安装网卡驱动。

   推荐在Windows内部安装 ``spice-guest-tools`` ，即安装QXL显卡驱动，极大提升SPICE的流畅度。

.. note::

   实践验证 :ref:`macos` 内置的 vnc  客户端访问安装程序的VNC界面存在问题，我尝试发现目前(2023年) 在 App Store中提供的 ``Remote Ripple`` 非常兼容。另外，也可以 ``tigervnc`` 客户端，能够兼容。
