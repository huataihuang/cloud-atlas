...
在 :ref:`incus_startup` 实践中，可以看到 ``incus admin init`` 交互过程会让你选择存储引擎，实际上这个存储引擎和 :ref:`docker_storage` 是相似的，能够支持 :ref:`zfs` :ref:`linux_lvm` :ref:`btrfs` 等不同的卷管理系统，以实现快照，clone的等高级功能。当然，最简单的形式还是 ``dir`` 即目录存储。
...
.. literalinclude:: incus_startup/zfs_containers
   :caption: ZFS存储的容器

实际上incus对ZFS的存储做了精细的划分，使用 ``zfs list`` 可以看到:

.. literalinclude:: incus_startup/zfs_list
   :caption: incus对ZFS的数据集做了精细划分

补充存储
=============

由于目前只有一块NVMe SSD存储(2T)，并且把大部分LV空间都划分给 ``incus-zpool`` 存储(1.4T)，这导致我存储数据空间不足。所以我还借用了 ``incus-zpool`` 创建了一个存储数据的目录:

.. literalinclude:: incus_zfs/docs
   :caption: 创建一个docs数据集存储数据
