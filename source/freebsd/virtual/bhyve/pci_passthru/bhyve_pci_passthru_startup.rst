...
我需要传递的设备是 :ref:`amd_mi50` (和 :ref:`tesla_p10` ) :

.. literalinclude:: bhyve_pci_passthru_startup/pciconf_vl_output
   :caption: 通过 ``pciconf`` 获取设备信息
   :emphasize-lines: 2-6,16-19

这里根据输出 ``vgapci0@pci0:4:0:0`` 可以知道这个 :ref:`amd_mi50` 的 ``bus/slot/function`` 值为 ``4/0/0`` (以及 :ref:`tesla_p10` 为 ``1/0/0`` )
...
.. literalinclude:: bhyve_pci_passthru_startup/pptdevs
   :caption: 设置屏蔽的直通的PCI设备( ``/boot/loader.conf`` ) 这里仅屏蔽 :ref:`amd_mi50`

如果屏蔽上述2个设备( ref:`amd_mi50` 和 :ref:`tesla_p10` )，则写成:

.. literalinclude:: bhyve_pci_passthru_startup/pptdevs_2
   :caption: 设置屏蔽的直通的PCI设备( ``/boot/loader.conf`` ) 这里同时屏蔽 :ref:`amd_mi50` :ref:`tesla_p10`
