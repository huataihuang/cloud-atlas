...
.. literalinclude:: jail_admin/chflags
   :caption: 使用 ``chflags`` 命令移除文件标志

- 然后删除jail所在目录，或者zfs销毁卷:

.. literalinclude:: jail_admin/zfs_destroy
   :caption: 删除zfs卷
...
.. literalinclude:: jail_admin/zfs_destroy_error
   :caption: 删除zfs卷报错
...
.. literalinclude:: jail_admin/lsof_output
   :caption: ``lsof`` 输出确实显示有访问存在
   :emphasize-lines: 3,4

可以看到十一个 ``1766`` 进程在使用中，这个 ``1766`` 进程如下:

.. literalinclude:: jail_admin/process_1766
