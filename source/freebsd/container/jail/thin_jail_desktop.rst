...
.. note::
   ...
   对 ``jail`` 的安装命令是在物理主机上执行 ``pkg -j mdev install ...`` 来执行，因为以下步骤也是物理主机的通用安装，所以直接使用了 ``pkg install ...`` ，实际可以采用临时性的 ``alias`` 命令::

      alias pkg="pkg -j mdev"

- :ref:`freebsd_init` 安装必要软件:

.. literalinclude:: ../../startup/freebsd_init/devops
   :caption: 安装运维软件

这里我遇到一个之前没有遇到过的报错信息，显示 ``pkg`` 在安装时需要 ``jail`` 中 ``/usr`` 目录下建立临目录，这导致和只读目录冲突:

.. literalinclude:: thin_jail_desktop/pkg_install_error
   :caption: 安装jail软件包时遇到只读目录冲突
   :emphasize-lines: 2

好奇怪啊，我对比了之前在 :ref:`vnet_thin_jail` 实践部署的 ``dev`` jail，同样是jail中 ``/usr`` 目录只读，但是执行 ``pkg -j dev install`` 完全没有问题，就好像根本没有使用 ``/usr`` 目录作为临时文件目录。WHY?
