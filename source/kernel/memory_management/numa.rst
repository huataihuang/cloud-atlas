...
NUMA(non-uniform memory access, 非一致性内存访问)是指在一个多处咯起系统中配置处理器的cluster方式，以便处理器核心能够就近访问共享内存，提高系统性能和系统的扩展性。NUMA在对称处理器(symmetric multiprocessing, SMP)系统中使用。SMP对称多处理器系统是一个"紧耦合"(tightly-coupled)"共享所有"(share
