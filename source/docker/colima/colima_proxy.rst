...
- 执行 ``colima start`` 启动colima虚拟机的时候，会自动将物理主机环境变量中有关代理配置设置注入虚拟机，这样所有在虚拟机中的服务都会继承这个代理配置，也就解决了 :ref:`across_the_great_wall`
- 在 ``colima`` 虚拟机内部启用 ``HTTP_PROXY`` 环境变量SHELL中使用 ``docker build`` (支持代理)
...
- ``colima start`` 通过HOST物理主机 ``HTTP_PROXY`` 配置注入虚拟机
- 配置 :ref:`docker_server_proxy` 和 :ref:`containerd_server_proxy` (不确定是否必须，待下次验证)
- 一定要在 ``colima`` 虚拟机内部，在 ``HTTP_PROXY`` 环境变量生效SHELL中使用 ``docker build`` (不能使用 ``nerdctl build`` )
