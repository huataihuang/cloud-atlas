...
.. note::
   ...
   自 2025 年底起，Chrome 已不再使用加密服务器名称指示 (ESNI)，而是过渡到其后继者加密客户端 Hello (ECH)。此功能在 Chrome 的最新版本中默认启用，通过加密 TLS 连接的整个初始握手过程来增强用户隐私。

ECH 的工作原理是将客户端 Hello 分为内部（加密）部分和外部（未加密）部分。外部部分会被发送到前端服务器（例如 CDN），前端服务器会使用其公钥解密内部部分并找到真正的目的地。这样可以防止任何在网络上“监听”的人看到正在连接的具体网站。

系统要求：ECH 依靠 DNS over HTTPS (DoH) 来安全地获取必要的加密密钥。这意味着如果您的系统或浏览器启用了 DoH，它将自动运行。

通过阻止网络观察者（例如互联网服务提供商 (ISP)）窥探用户的域名请求，ECH 是网络隐私保护的重要一步。
