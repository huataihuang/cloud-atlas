.. _install_llama_arm_kleidiai:

===========================
Arm KleidiAI架构安装Llama
===========================

``KleidiAI`` 是Arm处理器设计的优化AI工作负载的微架构软件库，可以增强使用Arm处理器后端的性能。Llama支持编译 KleidiAI 优化:

.. literalinclude:: install_llama_arm_kleidiai/build
   :caption: 使用 Arm CPU优化 ``-DGGML_CPU_KLEIDIAI=ON``

然后运行时验证是否支持了 KleidiAI:

.. literalinclude:: install_llama_arm_kleidiai/run
