...
.. note::
   ...
   模拟学习的项目名字是 「tweet」 (就像很多教学是以blog为案例)，虽然这个名字不能作为正式名，但是自己学习实践应该没有关系...

- 创建项目:

.. literalinclude:: rails_quickstart/new_tweet
   :caption: 执行 ``rails new`` 创建名为 ``tweet`` 的项目
...
.. literalinlude:: rails_quickstart/tweets_controller.rb
   :language: ruby
   :emphasize-lines: 1
