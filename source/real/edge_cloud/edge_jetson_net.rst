...
我的模拟云计算环境， :ref:`priv_cloud_infra` 采用一台二手到 :ref:`hpe_dl360_gen9` 构建的 :ref:`kvm_nested_virtual` ，所以这台物理服务器是稳定运行的关键基础。现代服务器都具备 :ref:`ipmi` 支持，可以通过主板集成的BMC进行管理。对于这台决定整个云计算模拟集群关键基础的服务器，需要有独立通道来访问带外管理，包括重启以及服务器终端访问。 我选择这台Jetson Nano主机来管理服务器带外:
 ...
 - 配置Nano的WiFi连接局域网
 - :strike:`Nano的有线网络通过交换机` 改为使用独立笔记本 :ref:`thinkpad_x220` 和 :ref:`hpe_dl360_gen9` 连接，通过 :ref:`conman` 来管理终端
 - 配置 :strike:`Nano主机` SSH tunnel端口转发，可以从外部直接访问服务器 :ref:`hp_ilo` 的WEB管理管控服务器运维，包括升级系统
...
- 增加连接到 ``home`` 热点(WPA认证)的网络连接(采用静态IP地址，因为我希望通过jetson nano的无线网络连接集成到 :ref:`k3s` 集群):

.. literalinclude:: ../../linux/ubuntu_linux/network/networkmanager/nmcli_wifi_wpa-psk_static_ip
   :language: bash
   :caption: nmcli添加wpa-psk认证wifi静态IP
