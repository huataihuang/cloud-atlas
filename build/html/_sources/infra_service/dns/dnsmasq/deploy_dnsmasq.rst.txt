...
.. note::
   ...
   我这里采用 ``listen-address=192.168.6.200,192.168.7.200,192.168.8.200,127.0.0.1`` 是因为我这个服务将针对整个 :ref:`priv_cloud_infra` ，并且我会在多个接口上使用这个网段的IP
...
- 可以为不同网段设置不同的域名，例如我构建了 ``huatai.me`` 和 ``cloud-atlas.io`` 两个域名(3个)::
...
这样我在 :ref:`priv_cloud_infra` 构建了 ``z-k8s`` (192.168.6.x) 和 ``y-k8s`` (192.168.8.x) 分别位于 ``staging.huatia.me`` 和 ``staging.cloud-atlas.io`` 两个域名下
